...
namespace AQOONHUB_SMS.Modules.Reports
{
 public sealed partial class ReportsRepository
 {
  ...
  public DataRow GetExportRecord(int id,int uid,string role){var d=ExecuteDataTable("SELECT TOP 1 ReportExportID,ReportKey,ReportName,Category,ExportFormat,FilterSummary,FileName,FilePath,FileSize,Status,GeneratedBy,GeneratedAt,ExpiresAt FROM dbo.ReportExports WHERE ReportExportID=@id AND (GeneratedBy=@u OR @admin=1)",new[]{P("@id",id),P("@u",uid),P("@admin",ReportAuthorization.IsStage5Admin(role))});return d.Rows.Count==0?null:d.Rows[0];}
